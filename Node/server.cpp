...
ConnectionResult Server::respondHandshake(string dest_ip, uint16_t dest_port) {
  ...
  while (retries-- > 0) {
    try {
      ...
      commandLine('i', "[Handshake] [S=" + std::to_string(sequence_num_first) +
      ...
      commandLine('i', "[Handshake] [S=" + std::to_string(sequence_num_second) +
                           "] [A=" + std::to_string(ack_num_second) +
                           "] Sending SYN-ACK request to " + dest_ip + ":" +
                           std::to_string(destPort));
      connection->sendSegment(ack(sequence_num_second, ack_num_second), dest_ip,
                              dest_port);
      ...
      commandLine('i', "[Handshake] [A=" + std::to_string(ack_num_third) +
      ...
      if (ack_num_third = sequence_num_second + 1) {
        commandLine('i', "Sending input to " + dest_ip + ":" +
                             std::to_string(destPort));
        return ConnectionResult(true, destIP, destPort, sequence_num_second,
                                ack_num_third);
      }
  ...
  commandLine('!', "[ERROR] [HANDSHAKE] Handshake failed after " +
